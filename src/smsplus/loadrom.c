...
void set_rom_config(void)
{
  ...
  bios.enabled &= 2;
  if (IS_SMS) bios.enabled |= option_sms.use_bios;

  /* force settings if AUTO is not set*/
  if (option_sms.console == 1)
    sms.console = CONSOLE_SMS;
  else if (option_sms.console == 2)
    sms.console = CONSOLE_SMS2;
  else if (option_sms.console == 3)
    sms.console = CONSOLE_GG;
  else if (option_sms.console == 4)
    sms.console = CONSOLE_GGMS;
  else if (option_sms.console == 5)
  {
    sms.console = CONSOLE_SG1000;
    cart.mapper = MAPPER_NONE;
  }
  else if (option_sms.console == 6)
  {
  ...
  if (option_sms.country == 1) /* USA */
  {
    sms.display = DISPLAY_NTSC;
    sms.territory = TERRITORY_EXPORT;
  }
  else if (option_sms.country == 2) /* EUROPE */
  {
    sms.display = DISPLAY_PAL;
    sms.territory = TERRITORY_EXPORT;
  }
  else if (option_sms.country == 3) /* JAPAN */
  {
    sms.display = DISPLAY_NTSC;
    sms.territory = TERRITORY_DOMESTIC;
  }
