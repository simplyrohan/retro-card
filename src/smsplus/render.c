...
void render_reset(void)
{
  ...
  /* Clear display bitmap_sms */
  memset(bitmap_sms.data, 0, bitmap_sms.pitch * bitmap_sms.height);
...
{
  int view = 1;
  int overscan = option_sms.overscan;
  ...
  top_border = top_border + (vdp.height - bitmap_sms.viewport.h) / 2;
  ...
  /* Vertical borders */
  if ((vline < top_border) || (vline >= (bitmap_sms.viewport.h + top_border)))
  {
    ...
    view = 0;
    if (overscan && (vline < (bitmap_sms.viewport.h + 2*bitmap_sms.viewport.y)))
    {
      /* Background color */
      memset(linebuf, BACKDROP_COLOR, bitmap_sms.viewport.w + 2*bitmap_sms.viewport.x);
      view = 1;
    }
  }
  /* Active display */
  else
  {
    ...
    if (vdp.reg[1] & 0x40)
    {
      ...
      if (overscan)
      {
        ...
        memset(linebuf - 14, BACKDROP_COLOR, bitmap_sms.viewport.x);
        memset(linebuf - 14 + bitmap_sms.viewport.w + bitmap_sms.viewport.x, BACKDROP_COLOR, bitmap_sms.viewport.x);
      }
    }
    else
    {
  ...
  if (view && !skip_render)
  {
    ...
    memcpy(
      bitmap_sms.data + (vline * bitmap_sms.pitch),
      internal_buffer,
      bitmap_sms.viewport.w + 2*bitmap_sms.viewport.x
    );
  }
}
...
void palette_sync(int index)
{
  ...
  // color = color << 8 | color >> 8;; // Absolutly no idea why this was here and it made all the colors wrong 
