...
static void viewport_check(void)
{
  ...
  /* update display area */
  if ((sms.console != CONSOLE_GG) || option_sms.extra_gg)
  {
    if(bitmap_sms.viewport.h != vdp.height)
    {
      bitmap_sms.viewport.oh = bitmap_sms.viewport.h;
      bitmap_sms.viewport.h = vdp.height;
      bitmap_sms.viewport.changed = 1;
    }
  }
  else
  {
    /* GG display area is fixed */
    bitmap_sms.viewport.h = 144;
  }

  /* update border area */
  bitmap_sms.viewport.y = 0;
  if (option_sms.overscan)
  {
    int max = sms.display ? 288 : 240;
    bitmap_sms.viewport.y = (max - bitmap_sms.viewport.h) / 2;
  }
...
void vdp_init(void)
{
  ...
  if ((sms.console == CONSOLE_GG) && (!option_sms.extra_gg))
  {
    bitmap_sms.viewport.w = 160;
    bitmap_sms.viewport.x = 48;
  }
  else
  {
    bitmap_sms.viewport.w = 256;
    bitmap_sms.viewport.x = 0;
  }

  /* overscan area */
  if (option_sms.overscan)
  {
    bitmap_sms.viewport.x += 14;
  }
