...
{
   // Memory.RAM   = (uint8_t*) calloc(RAM_SIZE, 1); // 131072
   Memory.RAM   = (uint8_t*) ps_calloc(RAM_SIZE, 1); // 131072
   Memory.SRAM  = (uint8_t*) calloc(SRAM_SIZE, 1); // 65536
   Memory.VRAM  = (uint8_t*) calloc(VRAM_SIZE, 1); // 65536
   // Memory.ROM   = (uint8_t*) calloc(MAX_ROM_SIZE + 0x200, 1); // I'm manually doing this
   Memory.FillRAM = (uint8_t*) calloc(0x8000, 1); // 32768
   // Memory.ROM_AllocSize = MAX_ROM_SIZE + 0x200; // I'm manually doing this

   Memory.Map = (uint8_t**)calloc(MEMMAP_NUM_BLOCKS, sizeof(uint8_t*)); // 4096
   Memory.MapInfo = (SMapInfo*)calloc(MEMMAP_NUM_BLOCKS, sizeof(SMapInfo)); // 4096 * SMapInfo

   IPPU.ScreenColors = (uint16_t *)calloc(256 * 9, sizeof(uint16_t)); // 2304
   IPPU.DirectColors = IPPU.ScreenColors + 256;
   // IPPU.TileCache = (uint8_t*) calloc(MAX_2BIT_TILES, 128); // 524288
   IPPU.TileCache = (uint8_t*) ps_calloc(MAX_2BIT_TILES, 128); // 524288
   IPPU.TileCached = (uint8_t*) calloc(MAX_2BIT_TILES, 1); // 4096

   bytes0x2000 = (uint8_t *)calloc(0x2000, 1); // 8192
   ...
   {
      if (!Memory.RAM) printf("Failed to allocate RAM\n");
      if (!Memory.SRAM) printf("Failed to allocate SRAM\n");
      if (!Memory.VRAM) printf("Failed to allocate VRAM\n");
      // if (!Memory.ROM) printf("Failed to allocate ROM\n");
      if (!Memory.FillRAM) printf("Failed to allocate FillRAM\n");
      if (!Memory.Map) printf("Failed to allocate Map\n");
      if (!Memory.MapInfo) printf("Failed to allocate MapInfo\n");
      if (!IPPU.ScreenColors) printf("Failed to allocate ScreenColors\n");
      if (!IPPU.TileCache) printf("Failed to allocate TileCache\n");
      if (!IPPU.TileCached) printf("Failed to allocate TileCached\n");
      if (!bytes0x2000) printf("Failed to allocate bytes0x2000\n");
