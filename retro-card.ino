...
IRAM_ATTR static void PS4_JOY()
{
    ...
    if ((num_get != -1) && buf[0] == 161)
    { // If connected then read JoyStick
...
uint8_t get_pad0(void)
{
    ...
    if ((value & JOY_UP) && (value & JOY_DOWN))
        value &= ~(JOY_UP | JOY_DOWN);
    if ((value & JOY_LEFT) && (value & JOY_RIGHT))
        value &= ~(JOY_LEFT | JOY_RIGHT);
...
{
    if (Y < DEFAULT_HEIGHT && X < DEFAULT_WIDTH)
...
{
    if (DEBUG)
        Serial.print("Timer install, freq=");
    if (DEBUG)
...
{
    // Serial.println("lcd_write_frame hehe");
    ...
    for (i = 0; i < height / 2; i++)
    {
        ...
        for (X_ = 0; X_ < 256 / 2; X_++)
        {
            ...
            tft.writePixel(X_ + 64, i, nes_16bit[(0x3F & ((uint8_t *)SCREENMEMORY[yc])[xc])]);
            // tft.writePixel(X_ * 2 + 1, i + 1, nes_16bit[(0x3F & ((uint8_t *)SCREENMEMORY[i * 2 + 1])[X_ * 2 + 1])]);
        }
    }
    tft.endWrite();
    if (DEBUG)
    ...
    // move screenmemory to oldscreenmemory
    // memcpy(OLDSCREENMEMORY, SCREENMEMORY, sizeof(SCREENMEMORY));
}
//================================================================================
//--------------------------------------------------------------------------------
/// QueueHandle_t vidQueue;
...
char *NESMENU()
{
    ...
    while (num < nMaxfiles && file.openNext(&dirFile, O_READ))
    {
        ...
        if (!file.isSubDir() && !file.isHidden())
        {
            ...
            if ((fileext[num][0] == 'N' || fileext[num][0] == 'n') && (fileext[num][1] == 'E' || fileext[num][1] == 'e') && (fileext[num][2] == 'S' || fileext[num][2] == 's'))
            {
                num++;
            }
        }
...
#define FILESPERPAGE 8
    ...
    while (1)
    {
        ...
        if (JOY_DOWN == 1)
        {
            if (CURSOR % FILESPERPAGE == FILESPERPAGE - 1 || CURSOR == loadedFileNames - 1)
                NamesDisplayed = false; // changed page
    ...
    *rom += ROM_BANK_LENGTH * rominfo->rom_banks;
    ...
    else
    {
        if (NULL == rominfo->vram)
            rominfo->vram = (uint8_t *)malloc(VRAM_LENGTH);
        if (NULL == rominfo->vram)
        {
...
int rom_getheader(unsigned char **rom, rominfo_t *rominfo)
{
    ...
    *rom += sizeof(head);
    ...
    {
        if (DEBUG)
            Serial.println("ROM is not a valid NES image");
        if (DEBUG)
            tft.println("ROM is not a valid NES image");
        return -1;
    }
...
#define COMBINE_FLAGS() ((n_flag & N_FLAG) | (v_flag ? V_FLAG : 0) | R_FLAG | (b_flag ? B_FLAG : 0) | (d_flag ? D_FLAG : 0) | (i_flag ? I_FLAG : 0) | (z_flag ? 0 : Z_FLAG) | c_flag)
...
nes_t *nes_create(void)
{
    ...
    if (NULL == machine->cpu->mem_page[0])
        machine->cpu->mem_page[0] = (uint8_t *)malloc(NES_RAMSIZE);
    if (NULL == machine->cpu->mem_page[0])
        goto _fail;
